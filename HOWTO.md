...
The most up-to date version of this document is available at:
    ...
    https://github.com/capricoin/electrum-capricoin-server/blob/master/HOWTO.md
...
document, we assume that user is called 'capricoin'). We also assume the
capricoin user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run capricoind and keep a copy of the full blockchain, 
which is roughly 4 GB in July 2015. If you have less than 2 GB of RAM 
make sure you limit capricoind to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running capricoind and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `capricoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser capricoin --disabled-password
    $ sudo apt-get install git
    $ sudo su - capricoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/capricoin/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download capricoind

We currently recommend capricoind 0.10.2.2 stable.

If you prefer to compile capricoind, here are some pointers for Ubuntu:
    ...
    $ sudo su - capricoin
    $ cd ~/src && git clone https://github.com/capricoin-project/capricoin.git
    $ cd capricoin/src
    $ make -f makefile.unix
    $ strip capricoind
    $ cp -a capricoind ~/bin

### Step 3. Configure and start capricoind

In order to allow Electrum to "talk" to `capricoind`, we need to set up an RPC
username and password for `capricoind`. We will then start `capricoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.capricoin
    $ $EDITOR ~/.capricoin/capricoin.conf

Write this in `capricoin.conf`:
...
If you have an existing installation of capricoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ capricoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `capricoind`:

    $ capricoind

Allow some time to pass, so `capricoind` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ capricoind getinfo

Before starting the electrum server your capricoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start capricoind at boot
time, running as the 'capricoin' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/capricoin/electrum-capricoin-server.git
    $ cd electrum-capricoin-server
    $ sudo ./configure
...
http://electrum1.capricoin.org/leveldb-dump/
...
Electrum reads a config file (/etc/electrum-capricoin.conf) when starting up. This
file includes the database setup, capricoind RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-capricoin.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-capricoin-server.
If you're using user capricoin to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "capricoin hard nofile 65536" >> /etc/security/limits.conf
     echo "capricoin soft nofile 65536" >> /etc/security/limits.conf
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u capricoin -i ulimit -n


Two more things for you to consider:

1. To increase security you may want to close capricoind for incoming connections and connect outbound only

2. Consider restarting capricoind (together with electrum-capricoin-server) on a weekly basis to clear out unconfirmed
...
The magic moment has come: you can now start your Electrum server as root (it will su to your unprivileged user):
    ...
    # electrum-capricoin-server start

Note: If you want to run the server without installing it on your system, just run 'run_electrum_capricoin_server" as the
unprivileged user.
...
If you want to stop Electrum server, use the 'stop' command:
    ...
    # electrum-capricoin-server stop


If your system supports it, you may add electrum-capricoin-server to the /etc/init.d directory. 
...
safely whenever your machine is rebooted.
    ...
    # ln -s `which electrum-capricoin-server` /etc/init.d/electrum-capricoin-server
    # update-rc.d electrum-capricoin-server defaults
...
capricoins to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-capricoin and we'll try to congratulate you
...
https://groups.google.com/forum/#!forum/electrum-capricoin-server
